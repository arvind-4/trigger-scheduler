    ...
    def validate(self, data):
        ...
        if trigger_type == 'scheduled':
            ...
            if interval_type in ['minutes', 'hours'] and not interval_value:
                raise serializers.ValidationError("Interval value is required for minutes/hours")
            if interval_type == 'fixed_time' and not fixed_time:
                raise serializers.ValidationError("Fixed time is required for fixed_time interval type")
                
        elif trigger_type == 'api':
            if not data.get('endpoint'):
                raise serializers.ValidationError("Endpoint is required for API triggers")
            if not data.get('payload_schema'):
                raise serializers.ValidationError("Payload schema is required for API triggers")
